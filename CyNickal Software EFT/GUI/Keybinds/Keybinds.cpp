...
void Keybinds::Render()
{
	...
	{
		int winVer = c_keys::GetWindowsVersion();
		std::string winVerStr = std::format("Keybinds Initialized - Windows Version: {} | {}", (winVer >= 22000) ? "11" : "10", winVer);
		ImGui::TextColored(ImVec4(0.0f, 1.0f, 0.0f, 1.0f), winVerStr.c_str());
	}
	else
		ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f), "Keybinds Not Initialized");

	if (ImGui::Button("Reinitialize Keyboard"))
	{
		c_keys::InitKeyboard(DMA_Connection::GetInstance());
	}
